...
extern float level_0_width;
extern float level_0_height;
extern float level_1_width;
extern float level_1_height;
extern float level_2_width;
extern float level_2_height;
extern float level_3_width;
extern float level_3_height;
extern float level_4_width;
extern float level_4_height;
extern float level_5_width;
extern float level_5_height;
extern float level_6_width;
extern float level_6_height;
extern float level_7_width;
extern float level_7_height;
extern float level_8_width;
extern float level_8_height;
extern float level_9_width;
extern float level_9_height;
extern float level_10_width;
extern float level_10_height;
extern float level_11_width;
extern float level_11_height;
extern float level_12_width;
extern float level_12_height;
...
static const vector<t_color> predef_colors = {
    /*my color*/
     ...
     /*my color*/
    //t_color(0x08, 0xBC, 0xE9), //light blue
    t_color(0xdb, 0xdb, 0xdb),//buildinggrey
    t_color(0xcc, 0xcc, 0xff),//HOTELBLUE
    t_color(0xcc, 0x99, 0xcc),//HIGHWAYPURPLE
    t_color(0xff, 0xff, 0xb2), //sandyellow
    //#f4f4f4  Very light gray (mostly white).
    //65ca00 strong green
    //52b573  Moderate cyan - lime green.
    //#ffffed : Very pale (mostly white) yellow.
    //90ee90 : Very soft lime green.
    t_color(0xf1, 0xf1, 0xf1),//lightlightgrey
    t_color(0x80, 0xBF, 0xFF), //"lakeColor"
    t_color(0x90, 0xee, 0x90), //"grassGreen"
    t_color(0xf9, 0xa1, 0x1b), //"pathOrange"
    t_color(0x9a, 0x9a, 0x9a), //roadGrey
    t_color(0x73, 0xae, 0x73), //golfgreen
    t_color(0x69, 0x69, 0x69), //segmentdarkgrey
    //f9a11b vivd orange, for highlight path
    t_color(0xf2, 0xef, 0xe9), // groundlightgray
    t_color(0x85, 0xbb, 0x65), //dollar green
    t_color(0xff, 0xd7, 0x00), //dollar gold
   
    t_color(0xdb, 0x86, 0x06), //foodYellow
...
void newfunction(void)
{
    if(zoom_switch==false)
    {
        if(scale_fit==true)
        {
            start_test = getIntersectionPosition(starting_intersection);
            end_test = getIntersectionPosition(ending_intersection);
            
            //lon x=cosavg
            start1_test.x=start_test.lon*(double)singleton->get_cosAvg();
            start1_test.y=start_test.lat;
            end1_test.x=end_test.lon*(double)singleton->get_cosAvg();
            end1_test.y=end_test.lat;
            t_bound_box frame(start1_test,end1_test);
            
            if(abs(frame.get_width())>abs(frame.get_height()))
            {
                longest = abs(frame.get_width());
            }
            else if(abs(frame.get_width())<abs(frame.get_height()))
            {
                longest = abs(frame.get_height());
            }
           
            if(longest<level_0_width && longest<level_0_height)
            {
                if(longest<level_1_width && longest<level_1_height)
                {
                    while(zoom_level<1)
                    {
                       newfunction_helper();
            ...
            if(longest<level_1_width && longest<level_1_height)
            {
                if(longest<level_2_width && longest<level_2_height)
                {
                    while(zoom_level<2)
                    {
                       newfunction_helper();
            ...
            if(longest<level_2_width && longest<level_2_height)
            {
                if(longest<level_3_width && longest<level_3_height)
                {
                    while(zoom_level<3)
                    {
                       newfunction_helper();
            ...
            if(longest<level_3_width && longest<level_3_height)
            {
                if(longest<level_4_width && longest<level_4_height)
                {
                    while(zoom_level<4)
                    {
                       newfunction_helper();
            ...
            if(longest<level_4_width && longest<level_4_height)
            {
                if(longest<level_5_width && longest<level_5_height)
                {
                    while(zoom_level<5)
                    {
                       newfunction_helper();
            ...
            if(longest<level_5_width && longest<level_5_height)
            {
                if(longest<level_6_width && longest<level_6_height)
                {
                    while(zoom_level<6)
                    {
                       newfunction_helper();
            ...
            if(longest<level_6_width && longest<level_6_height)
            {
                if(longest<level_7_width && longest<level_7_height)
                {
                    while(zoom_level<7)
                    {
                       newfunction_helper();
            ...
            if(longest<level_7_width && longest<level_7_height)
            {
                if(longest<level_8_width && longest<level_8_height)
                {
                    while(zoom_level<8)
                    {
                       newfunction_helper();
            ...
            if(longest<level_8_width && longest<level_8_height)
            {
                if(longest<level_9_width && longest<level_9_height)
                {
                    while(zoom_level<9)
                    {
                       newfunction_helper();
            ...
            if(longest<level_9_width && longest<level_9_height)
            {
                if(longest<level_10_width && longest<level_10_height)
                {
                    while(zoom_level<10)
                    {
                       newfunction_helper();
            ...
            if(longest<level_10_width && longest<level_10_height)
            {
                if(longest<level_11_width && longest<level_11_height)
                {
                    while(zoom_level<11)
                    {
                       newfunction_helper();
            ...
            if(longest<level_11_width && longest<level_11_height)
            {
                ...
                if(longest<level_12_width && longest<level_12_height)
                {
                    while(zoom_level<11)
                    {
                       newfunction_helper();
...
void newfunction_helper(void)
{
    t_point start1 = singleton->get_intersectionPositionIn_xy_coordinate(starting_intersection);
    t_point end1 = singleton->get_intersectionPositionIn_xy_coordinate(ending_intersection);
    t_bound_box frame(start1,end1);
    t_point center = frame.get_center();

    zoom_level++;
    trans_coord.xleft = center.x - (center.x - trans_coord.xleft) / (float) ZOOM_FACTOR;
    trans_coord.xright = center.x + (trans_coord.xright - center.x) / (float) ZOOM_FACTOR;
    trans_coord.ytop = center.y - (center.y - trans_coord.ytop) / (float) ZOOM_FACTOR;
    trans_coord.ybot = center.y + (trans_coord.ybot - center.y) / (float) ZOOM_FACTOR;
    update_transform();
    if(zoom_level<0)
    {     
        if (drawscreen != NULL) 
...
int init_postscript(const char *fname) {
    ...
    /*MY to do color*/
     fprintf(gl_state.ps, "/sandyellow {.93,.93,78 setrgbcolor} def\n");
     
    fprintf(gl_state.ps, "/lakecolor {.5,.75,1 setrgbcolor} def\n");
    fprintf(gl_state.ps, "/grassgreen {.56, .93, .56  setrgbcolor} def\n");
    fprintf(gl_state.ps, "/pathorange {.98, .63, .11 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/roadgrey { .6, .6, .6 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/golfgreen { .45, .68, .45 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/segmentdarkgrey { .41, .41, .41 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/dollargreen { .52, .73, .40 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/dollargold { .52, .73, .40 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/groundlightgray { .100, .84, .0 setrgbcolor } def\n");
    fprintf(gl_state.ps, "/foodyellow { .85, .53, .2 setrgbcolor } def\n");
...
//    menu_attributes.event_mask = 0;
    ...
    /*strcpy(button_state.button[9].text, "Find");
    strcpy(button_state.button[10].text, "Find Street");
    strcpy(button_state.button[11].text, "Find Feature");
    strcpy(button_state.button[12].text, "Find POI");
    strcpy(button_state.button[13].text, "Transport Off");
    strcpy(button_state.button[14].text, "Clear");
    strcpy(button_state.button[15].text, "Switch");
    strcpy(button_state.button[16].text, "---1");*/
    ...
    /*button_state.button[9].fcn = act_on_find_intersection;
    button_state.button[10].fcn = act_on_search_street;
    button_state.button[11].fcn = act_on_feature;
    button_state.button[12].fcn = act_on_POI;
    button_state.button[13].fcn = act_on_station;
    button_state.button[14].fcn = act_on_clear;
    button_state.button[15].fcn = act_on_switch_map;
    button_state.button[16].fcn = NULL;*/
